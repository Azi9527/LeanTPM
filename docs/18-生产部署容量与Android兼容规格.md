...
| 场景 | 适用规模 | 应用层 | 数据库 | 持久安全状态 | 附件与备份 |
|---|---|---|---|---|---|
| 开发/演示（非生产） | ≤10 并发、≤500 台设备 | 4 vCPU / 8GB，可与 MySQL 共机 | 共享实例，数据盘 200GB SSD | MySQL 独立表与索引 | 附件 500GB；不承诺高可用 |
| 小型单机生产 | ≤30 并发、≤1500 台设备 | 1 台 8 核 / 32GB ECC，应用与数据库逻辑隔离 | MySQL 8，Buffer Pool 10～12GB，独立 500GB NVMe/企业 SSD | 纳入数据库备份与容量监控 | 附件 4TB RAID1；独立备份 6TB |
| 标准生产（推荐） | ≤100 并发、≤5000 台设备 | 2 台，每台 4 vCPU / 8GB；或物理机虚拟化等价资源 | 1 台 8 vCPU / 32GB ECC，独立 500GB～1TB NVMe/企业 SSD；建议增加同规格只读/灾备副本 | 纳入数据库复制、备份和告警 | 附件 8TB 可用 RAID10/NAS；独立备份 ≥12TB |
| 高可用扩展 | 100～300 并发、5000～20000 台设备 | 3 台，每台 8 vCPU / 16GB | 主从各 16 vCPU / 64GB，数据卷 1～2TB NVMe，半同步复制 | 随主库复制并定期清理过期状态 | 双控制器 NAS/对象存储 20TB 起；异地备份按 1.5 倍在线量 |
...
| 安全状态 | 会话、令牌撤销、登录锁定和请求幂等写入 MySQL；禁止用进程内临时状态替代 |
...
- 应用 18080、MySQL 3306 和 SSH 22 仅允许管理网或明确的安全组来源；18080、3306 不向公网开放。
- 正式证书必须包含 Android 使用的完整中间证书链，禁止自签名证书直接下发普通终端。
- MySQL 使用独立最小权限账号；密钥由 Secret 管理，禁止写入 Git、镜像和日志。

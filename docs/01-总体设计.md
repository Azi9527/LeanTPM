...
部署初期为一个前端应用、一个后端应用和一个 MySQL 数据库。认证会话、令牌撤销、登录锁定和请求幂等状态均使用 MySQL 持久化，主机重启后仍保持安全语义；文件通过统一附件服务落本地目录，接口保持与未来对象存储兼容。
...
本地启动方式见仓库 `README.md`。生产部署建议使用反向代理提供 HTTPS，将数据库、JWT 密钥和文件目录全部通过环境变量或密钥管理注入。

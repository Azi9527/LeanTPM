...
import { maintenanceApi, type SchemeRow, type TaskDetail, type TaskRow, type TaskStatus } from '@/api/maintenance'
...
const statusMeta: Record<TaskStatus, { label: string; type: '' | 'success' | 'warning' | 'danger' | 'info' }> = {
  PENDING_ASSIGNMENT: { label: '待派工', type: 'info' },
  PENDING: { label: '待执行', type: 'info' },
  IN_PROGRESS: { label: '执行中', type: 'warning' },
  PAUSED: { label: '已暂停', type: 'warning' },
  PENDING_CONFIRMATION: { label: '待确认', type: '' },
...
async function loadReferences() {
  try {
    const [equipmentPage, schemePage, userRows] = await Promise.all([
      ...
      maintenanceApi.schemes({ status: 1, page: 1, pageSize: 200 }),
...
async function assign(row: TaskRow) {
  ...
  try {
    await maintenanceApi.assignTask(row.id, {
...
async function confirm(row: TaskRow, approved: boolean) {
  const value = await ElMessageBox.prompt(
    approved ? '可填写确认意见' : '请输入退回原因',
    approved ? '确认完成' : '退回执行',
    { inputPattern: approved ? undefined : /\S+/, inputErrorMessage: '驳回原因不能为空' },
  )
  try {
    await maintenanceApi.confirmTask(row.id, { approved, comment: value.value || null, version: row.version })
...
async function close(row: TaskRow, targetStatus: 'CANCELLED' | 'VOIDED') {
  ...
  try {
    await maintenanceApi.closeTask(row.id, targetStatus, { reason: value.value, version: row.version })
...
<template>
  <div class="page-shell">
    <header class="page-header">
      <div><h1>维保任务</h1><p>统一管理派工、转派、协同、执行、暂停、确认、备件耗用和全量事件追踪。</p></div>
      <el-button v-if="auth.can('maintenance:task:create')" type="primary" @click="openCreate">创建任务</el-button>
    </header>
    ...
    <section class="surface-card table-card" v-loading="loading">
      <div class="table-toolbar"><span class="table-title">维保任务台账</span><span>共 {{ total }} 条</span></div>
      <el-table :data="rows" row-key="id">
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="auth.can('maintenance:task:assign') && ['PENDING_ASSIGNMENT','PENDING','IN_PROGRESS','PAUSED','OVERDUE'].includes(row.taskStatus)" link type="primary" @click="assign(row)">{{ row.assigneeUserId ? '转派' : '派工' }}</el-button>
            <template v-if="auth.can('maintenance:task:confirm') && row.taskStatus === 'PENDING_CONFIRMATION'">
              <el-button link type="success" @click="confirm(row, true)">确认</el-button>
              <el-button link type="warning" @click="confirm(row, false)">退回</el-button>
            </template>
            <el-dropdown v-if="auth.can('maintenance:task:cancel') && !['COMPLETED','CANCELLED','VOIDED'].includes(row.taskStatus)">
    ...
    <el-dialog v-model="createVisible" title="创建手工维保任务" width="min(700px, 96vw)">
    ...
    <el-drawer v-model="detailVisible" :title="detail?.task.taskCode" size="min(860px, 97vw)">
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="有效工时">{{ detail.task.effectiveWorkMinutes }} 分钟</el-descriptions-item>
          <el-descriptions-item label="暂停时长">{{ Math.round(detail.task.totalPausedSeconds / 60) }} 分钟</el-descriptions-item>
          <el-descriptions-item label="协同人员">{{ detail.collaborators.map((item) => item.userName).join('、') || '无' }}</el-descriptions-item>
          <el-descriptions-item label="备件成本">¥ {{ Number(detail.task.materialCost || 0).toFixed(2) }}</el-descriptions-item>
        </el-descriptions>
        <h3>项目结果</h3>
        <el-table :data="detail.items" size="small"><el-table-column prop="itemName" label="项目" min-width="160" /><el-table-column prop="maintenanceStandard" label="标准" min-width="220" /><el-table-column label="结果" min-width="170"><template #default="{ row }">{{ row.result?.numericValue ?? row.result?.textValue ?? row.result?.selectedValue ?? row.result?.resultCode ?? '未填写' }}</template></el-table-column><el-table-column label="异常" width="80"><template #default="{ row }"><el-tag v-if="row.result?.abnormalFlag" type="danger">异常</el-tag><span v-else>—</span></template></el-table-column></el-table>
        <h3>备件耗用</h3>
        <el-table :data="detail.materials" size="small"><el-table-column prop="materialCode" label="编码" /><el-table-column prop="materialName" label="名称" /><el-table-column label="数量"><template #default="{ row }">{{ row.quantity }} {{ row.unit }}</template></el-table-column><el-table-column prop="totalCost" label="金额" /></el-table>
        <h3>事件轨迹</h3>

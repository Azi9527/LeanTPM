...
import { inspectionApi, type Statistics } from '@/api/inspection'
...
async function load() {
  loading.value = true
  try {
    value.value = await inspectionApi.statistics()
...
<template>
  <div class="page-shell" v-loading="loading">
    <header class="page-header">
      <div><h1>点检统计</h1><p>聚焦今日达成、按时完成、逾期积压和异常闭环，指标受当前组织数据范围约束。</p></div>

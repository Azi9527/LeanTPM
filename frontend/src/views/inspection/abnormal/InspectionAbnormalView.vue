...
const statusMeta: Record<string, { label: string; type: 'danger' | 'warning' | 'success' | 'info' }> = {
  ...
  PROCESSING: { label: '已处理', type: 'success' },
...
async function openHandle(row: AbnormalRow) {
  ...
  Object.assign(form, {
    ...
    permanentCountermeasure: row.permanentCountermeasure || '',
...
async function saveHandle() {
  ...
  try {
    ...
    ElMessage.success('异常处置已保存，状态已更新为已处理')
    await load()
  } catch (error) {
    ElMessage.error(errorMessage(error))
  }
...
<template>
  <div class="page-shell">
    <header class="page-header"><div><h1>点检异常</h1><p>点检结果自动生成异常记录，记录原因分析、临时措施和恒久对策。</p></div></header>
    ...
    <section class="surface-card table-card" v-loading="loading">
      ...
      <el-table :data="rows" row-key="id" server-query @smart-query-change="applyTableQuery">
        ...
        <el-table-column prop="equipmentName" label="设备/项目" min-width="220"><template #default="{ row }">{{ row.equipmentCode }} · {{ row.equipmentName }}<div class="muted">{{ row.itemName }}</div></template></el-table-column>
        ...
        <el-table-column label="操作" smart-filter="none" width="280" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="auth.can('inspection:abnormal:handle') && row.abnormalStatus !== 'CLOSED'" link type="primary" @click="openHandle(row)">处理</el-button>
    ...
    <el-dialog v-model="detailVisible" :title="`异常详情 · ${selected?.abnormalCode || ''}`" width="min(780px, 96vw)">
      <template v-if="selected">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="设备">{{ selected.equipmentCode }} · {{ selected.equipmentName }}</el-descriptions-item>
          ...
          <el-descriptions-item label="原因分析" :span="2">{{ selected.causeAnalysis || '-' }}</el-descriptions-item>
          <el-descriptions-item label="临时措施" :span="2">{{ selected.temporaryAction || '-' }}</el-descriptions-item>
          <el-descriptions-item label="恒久对策" :span="2">{{ selected.permanentCountermeasure || '-' }}</el-descriptions-item>
        </el-descriptions>
    ...
    <el-dialog v-model="dialogVisible" class="abnormal-handle-dialog" :title="`处理异常 · ${selected?.abnormalCode || ''}`" width="min(1080px, 96vw)" align-center>
      <div class="abnormal-handle-body">
        ...
        <el-form label-position="top" class="abnormal-handle-form">
          ...
          <div class="measure-grid">
            <el-form-item label="原因分析" class="measure-field"><el-input v-model="form.causeAnalysis" type="textarea" :rows="4" resize="none" /></el-form-item>
            <el-form-item label="临时措施" class="measure-field"><el-input v-model="form.temporaryAction" type="textarea" :rows="4" resize="none" /></el-form-item>
            <el-form-item label="恒久对策" class="measure-field"><el-input v-model="form.permanentCountermeasure" type="textarea" :rows="4" resize="none" /></el-form-item>
          </div>
        </el-form>
        <section v-loading="attachmentLoading" class="abnormal-handle-attachments">
          <div class="attachment-section-head"><h3>现场附件</h3><span v-if="attachments.length">{{ attachments.length }} 个</span></div>
          <div v-if="attachments.length" class="attachment-viewport">
            <InspectionAttachmentList
              :attachments="attachments"
              :load-content="loadAttachmentContent"
            />
          </div>
          <span v-else class="attachment-empty-inline">暂无附件</span>
        </section>
      </div>
...
.abnormal-handle-body { display: grid; gap: 14px; }
.abnormal-handle-form { display: grid; gap: 14px; }
.assignment-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }
.measure-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }
.abnormal-handle-form :deep(.el-form-item) { margin-bottom: 0; }
.abnormal-handle-form :deep(.el-select),
.abnormal-handle-form :deep(.el-date-editor) { width: 100%; }
.measure-field :deep(.el-textarea__inner) { min-height: 106px !important; }
.abnormal-handle-attachments { display: grid; gap: 8px; padding-top: 10px; border-top: 1px solid var(--el-border-color-lighter); }
.attachment-section-head { display: flex; align-items: center; justify-content: space-between; }
.attachment-section-head h3 { margin: 0; font-size: 15px; }
.attachment-section-head span,
.attachment-empty-inline { color: var(--el-text-color-secondary); font-size: 13px; }
.attachment-viewport { max-height: 82px; overflow-y: auto; padding-right: 3px; }
@media (max-width: 800px) {
  .assignment-grid,
  .measure-grid { grid-template-columns: 1fr; }
  .attachment-viewport { max-height: none; overflow: visible; }
}
</style>
